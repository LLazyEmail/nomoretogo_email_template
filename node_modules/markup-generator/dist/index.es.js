...
const catchErrorTraceOutput = error => {
  ...
  callerLine.slice(index + 2, callerLine.length);
  throw error;
};
...
function checkWarnings(warnings) {
  lodash.exports.forEach(warnings, (index, element) => {
    if (index) {
...
function checkErrors(errors) {
  if (Object.values(errors).includes(false)) {
    lodash.exports.forEach(errors, (_, error) => {
      if (!errors[error]) {
...
export { catchErrorTraceOutput, checkErrors, checkHtml, checkWarnings, displayCLIErrors, generateTemplateName, isFolderExists, printMessage, readSourceFile, stateInit, writeHTML, writingFile };
